...
import { SpaceReserveForm } from '@/features/spaces/components/SpaceReserveForm'
...
export default async function SpaceReservePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      <Link href={`/spaces/${id}`} className="mb-6 inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-[#0A1628] transition-colors">
        <ArrowLeft className="h-3.5 w-3.5" /> 공간 상세로
      </Link>
      ...
      <div className="relative overflow-hidden mb-8 rounded-3xl border border-[#C9A961]/20 bg-linear-to-br from-[#050B1A] via-[#0A1628] to-[#0F2959] px-7 py-8 text-white">
        ...
        <div className="relative">
          ...
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">{space.name}</h1>
          {space.operating_hours && (
            <p className="text-sm text-slate-300/80 mt-2">운영시간: {space.operating_hours}</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        <SpaceReserveForm
          ...
          operatingHours={space.operating_hours ?? undefined}
        />
      </div>
    </div>
  )
